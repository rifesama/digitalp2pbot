...
describe("TestCases for DynamoDB commands", () => {
  ...
  it("should create the record using the updateItem method", async () => {
    const schema = await new MockSchema(docClient, collection);
    const uuid = faker.string.uuid();
    const body = {
      uuid: uuid,
    ...
    const item = await schema.updateItem(body);
    assert.equal(item?.name, "count");
    assert.equal(item?.lastName, body.lastName);
    assert.equal(item?.phone, body.phone);
    assert.equal(item?.address, body.address);
    assert.equal(item?.active, body.active);
    assert.equal(item?.PK, `${DynamoDBPrefix.USER}${uuid}`);
    assert.equal(item?.SK, `${DynamoDBPrefix.METADATA}`);
  });

  it("should create the record using the updateItem and update the data using beforeCreate", async () => {
    class mock extends MockSchema {
      async beforeCreate(data: Record<string, any>): Promise<void> {
        data.name = "test";
      }
  ...
  it("should create the record using the updateItem and update the data using onCreate", async () => {
    class mock extends MockSchema {
      async onCreate(
        data: Record<string, any>,
  ...
  it("should update the record using the updateItem", async () => {
    const schema = await new MockSchema(docClient, collection);
    const uuid = faker.string.uuid();
    const body = {
      PK: `${DynamoDBPrefix.USER}${uuid}`,
    ...
    const item = await schema.updateItem(bodySecondItem);
    assert.equal(item?.name, bodySecondItem.name);
    assert.equal(item?.lastName, bodySecondItem.lastName);
    assert.equal(item?.phone, bodySecondItem.phone);
    assert.equal(item?.address, bodySecondItem.address);
    assert.equal(item?.active, bodySecondItem.active);
    assert.equal(item?.PK, `${DynamoDBPrefix.USER}${uuid}`);
    assert.equal(item?.SK, `${DynamoDBPrefix.METADATA}`);
  });

  it("should update the record using the updateItem and update the data using beforeUpdate", async () => {
    class mock extends MockSchema {
      async beforeUpdate(
        item: Record<string, any>,
        data: Record<string, any>,
  ...
  it("should update the record using the updateItem and update the data using onUpdate", async () => {
    class mock extends MockSchema {
